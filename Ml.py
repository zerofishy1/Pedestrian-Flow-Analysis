...
try:
    while cap.isOpened():
        ...
        if frame_count % frame_skip == 0:
            ...
            cv2.putText(frame, f"People in last 10 sec: {current_person_count}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
    ...
    output_data = {
        ...
        "peak_hour_window": {
            "start_time": peak_window[0] if peak_window else None,
            "end_time": peak_window[1] if peak_window else None,
            "max_people": max_people
        },
        "tracked_ids": list(first_seen_objects),  # ID по порядку
    }

    with open("movement_tracking_results.json", "w", encoding="utf-8") as json_file:
